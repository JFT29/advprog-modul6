...
In this milestone, I simulated a slow-running request by adding a `/sleep` endpoint that triggers a 10-second delay using `thread::sleep`. This exercise was instrumental in demonstrating the inherent flaws of a single-threaded server architecture. When the server is processing the `/sleep` request, it remains entirely occupied and cannot accept or process any other incoming TCP connections until the sleep duration concludes. I observed this firsthand by attempting to load the root page in a second browser tab while the first was still waiting for the sleep route to respond. The second request was forced to wait, illustrating that the throughput of the server is capped by the processing time of the current request. This blocking behavior highlights why multithreading is a non-negotiable requirement for production-grade web servers, as it allows the system to remain responsive to other users even when handling long-running tasks.